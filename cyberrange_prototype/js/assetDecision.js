//Add default values for the cards
//add carousel for assets
//access database to read/change values
//BYOD???
    ...
    //console.log("ready state changed");
      ...
      for (var i = 0; i < data.length; i++) {
        if (data[i].assetType == "Computers") {
          ...
          var key = data[i].os_protocol.replace("(", "").replace(")", "").replace("-", "")
...
window.onbeforeunload= function() {
  ...
  for (var prop in Computers) {
    ...
    if (Computers[prop].delta != 0) {arr.push(Computers[prop]);}
    }
  }
  for (var prop in Laptops) {
    ...
    if (Laptops[prop].delta != 0) {arr.push(Laptops[prop]);}
  }
}
  for (var prop in Servers) {
    ...
    if (Servers[prop].delta != 0) {arr.push(Servers[prop]);}
  }
}
  for (var prop in networkDevices) {
    ...
    if (networkDevices[prop].delta != 0) {arr.push(networkDevices[prop]);}
  }
}
      ...
      if (obj.hasOwnProperty(property) && !(property == "getName")) {
        ...
        <div class="card shadow-sm text-center   "> \
            <div class="card-img-top text-secodary text-center shadow-sm p-3 mb-5 bg-white rounded"> '+ icons[obj.getName()] + ' '+ colors[i%3]+ ';" ></i> </div> \
                <div class="card-body"> \
                    <div class="Disptitle"> \
                    <h1 class="display-4 text-muted" style="font-size:20px"> <span id="'+"name"+i+'"></span></h1> \
                    <h2 class="display-6 text-muted" style="font-size:20px"> <span id="'+"op_sys"+i+'"></span></h2> \
                    </div> \
                    <p class="card-text"> \
                        <h1 class="display-2 mb-0"id="'+"qty"+i+'" style="font-size: 40px"></h1> \
                        Assets<br> \
